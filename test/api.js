...
test('vault.readdir', async t => {
  var vault = new DPackVault(testStaticDPackURL, {localPath: tempy.directory()})
...
test('vault.readFile', async t => {
  ...
  t.deepEqual(helloTxt, 'greetings martian')

  // read utf8 2
  var helloTxt2 = await vault.readFile('/subdir/greetings.txt', 'utf8')
  t.deepEqual(helloTxt2, 'greetings')
  ...
  var badVault = new DPackVault(fakeDPackURL, {localPath: tempy.directory()})
...
test('DPackVault.load', async t => {
  // create it
  var loadedVault = await DPackVault.load({
    localPath: createdVault._localPath
  })

  // check the dpack.json
  var manifest = JSON.parse(await loadedVault.readFile('dpack.json'))
  t.deepEqual(manifest.title, 'The Title')
  t.deepEqual(manifest.description, 'The Description')
  t.deepEqual(manifest.type, ['dataset'])
  t.deepEqual(manifest.author, {name: 'Bob', url: 'dweb://ffffffffffffffffffffffffffffffff'})
...
test('vault.writeFile', async t => {
  ...
  await dotest('beaker1.png', dbrowserPng, 'binary')
  await dotest('beaker2.png', dbrowserPng.toString('base64'), 'base64')
  await dotest('beaker3.png', dbrowserPng.toString('hex'), 'hex')
})
...
test('versioned reads and writes', async t => {
  ...
  var vault = await DPackVault.create({localPath: tempy.directory(), title: 'Another Test DPack'})
  ...
  // helper
  function checkout (v) {
    return new DPackVault(vault.url + v, {localPath: tempy.directory()})
  }
...
test('vault.createNetworkActivityStream', async t => {
  ...
  var vault = new DPackVault(testStaticDPack2URL, {localPath: tempy.directory()})
  await vault._loadPromise
